    ...
    <div className="space-y-6">
      <div>
        <div className="flex items-start gap-3 mb-4">
          ...
          <div>
            ...
            <p className="text-sm text-muted-foreground mt-1">
              Optional compliance details for regulated products (Firecrackers, Explosives, etc.)
            </p>
          </div>
        </div>
        ...
        <div className="flex items-start gap-2 p-3 bg-blue-50 border border-blue-200 rounded-lg">
          ...
          <p className="text-sm text-blue-900">
            These fields are optional but recommended for firecrackers and other regulated goods.
          </p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="space-y-4">
          <Label>Additional Compliance</Label>
          <div className="flex items-center justify-between border p-3 rounded-md">
            <Label htmlFor="isGreenCracker" className="cursor-pointer">Green Cracker Certified</Label>
            <Switch
              id="isGreenCracker"
              checked={formData.isGreenCracker}
              onCheckedChange={(checked) => onChange("isGreenCracker", checked)}
            />
          </div>

          <div className="flex items-center justify-between border p-3 rounded-md">
            <Label htmlFor="pesoApproved" className="cursor-pointer">PESO Approved</Label>
            <Switch
              id="pesoApproved"
              checked={formData.pesoApproved}
              onCheckedChange={(checked) => onChange("pesoApproved", checked)}
            />
          </div>

          <div className="flex items-center justify-between border p-3 rounded-md">
            <Label htmlFor="bariumFree" className="cursor-pointer">Barium Free</Label>
            <Switch
              id="bariumFree"
              checked={formData.bariumFree}
              onCheckedChange={(checked) => onChange("bariumFree", checked)}
            />
          </div>
        </div>

        <div className="space-y-2">
          <Label htmlFor="soundLevel">Sound Level Category</Label>
          <Select
            value={formData.soundLevel}
            onValueChange={(val) => onChange("soundLevel", val)}
          >
            <SelectTrigger>
              <SelectValue placeholder="Select sound level" />
            </SelectTrigger>
            <SelectContent>
              <SelectItem value="Low">Low (Fountains/Sparklers)</SelectItem>
              <SelectItem value="Medium">Medium (Spinners/Flower Pots)</SelectItem>
              <SelectItem value="High">High (Rockets/Bombs)</SelectItem>
            </SelectContent>
          </Select>
          <p className="text-sm text-muted-foreground mt-1">
            Categorize audible noise level for safety regulations (required for Green Crackers).
          </p>
        </div>
      </div>
