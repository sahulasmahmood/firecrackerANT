...
export function VariantsTab({
  ...
  const handleAddNewVariant = () => {
    const newVariant: ProductVariant = {
      ...
      // Removed variantColour, variantSize, variantMaterial
      customAttributes: [],
      variantGST: 0,
    ...
    const updatedVariants = formData.variants.map((v) => {
      if (v.id === variantId) {
        ...
        if (field === "variantSellingPrice") {
          const currentSellingPrice = parseFloat(value as string) || 0;
          ...
          const currentMRP = parseFloat(value as string) || 0;
          const prices = calculatePricesSmartly(
            variantId,
          ...
          const currentDiscount = parseFloat(value as string) || 0;
          const prices = calculatePricesSmartly(
            variantId,
            v.variantSellingPrice || 0,
            v.variantMRP || 0,
      ...
      img.onload = () => {
        ...
        canvas.toBlob((blob) => {
          if (blob) {
            ...
            link.download = `barcode-${barcode}-${variantName || "variant"
              }.png`;
  ...
  const handleSelectFromInventory = (productId: string, variantId: string) => {
    ...
    if (isAlreadyUsedInCurrentProduct) {
      ...
      toast.error(
        `This inventory item is already used in ${formData.enableVariants
              ...
              <div className="bg-gray-50 px-3 sm:px-6 py-2 sm:py-3 flex items-center justify-between border-b gap-2">
                ...
                <div className="flex items-center gap-1 sm:gap-2 flex-shrink-0">
                  {formData.enableVariants && (
                    ...
                    >
                      <Star
                        className={`h-3 w-3 sm:h-4 sm:w-4 ${variant.isDefault
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-3 sm:gap-4">
                  <div>
                    ...
                    <Input
                      ...
                      placeholder="e.g., 1000 Wala - Standard"
                  ...
                  <div>
                    ...
                    <Input
                      ...
                      placeholder="e.g., Box of 10, Single Piece, Packet"
                      className="mt-2 text-sm sm:text-base"
                    />
                    <p className="text-xs text-muted-foreground mt-1">
                  ...
                  {variant.customAttributes &&
                      ...
                      {variant.customAttributes.map((attr, attrIndex) => (
                        ...
                        >
                          <div className="flex-1 w-full grid grid-cols-1 sm:grid-cols-2 gap-2 sm:gap-3">
                            <Input
                              placeholder="Property name (e.g., Duration)"
                              value={attr.key}
                              onChange={(e) =>
                            ...
                            />
                            <Input
                              placeholder="Property value (e.g., 30 Seconds)"
                              value={attr.value}
                              onChange={(e) =>
                                handleUpdateCustomAttribute(
                                  variant.id,
                    ...
                    <div className="border-2 border-dashed rounded-lg p-4 sm:p-8 text-center bg-gray-50">
                      ...
                      <p className="text-xs text-muted-foreground">
                        Add product-specific details like duration, shot count, color effect, etc.
                      </p>
                    </div>
                  )}
