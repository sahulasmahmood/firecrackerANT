...
export function CartProvider({ children }: { children: ReactNode }) {
  ...
  const addToCart = async (product: Product, variantIndex: number, cuttingStyle?: string) => {
    ...
    if (!isAuthenticated || !user?.id) {
      toast.error('Please log in to add items to your Enquiry List');
    ...
    if (user?.role !== 'user') {
      toast.error('Enquiry List is only available for customers');
      return;
    }
    ...
    console.log('🛒 Add to enquiry list - Stock check:', {
    ...
    if (existingItem) {
      ...
      try {
        ...
        toast.success('Enquiry List updated');
      } catch (error) {
    ...
    const newItem: CartItem = {
      ...
      categoryId: '', // Product interface doesn't have categoryId, so defaulting to empty string
    };
    ...
    try {
      ...
      toast.error('Failed to add item to Enquiry List');
    }
  };
  ...
  const removeFromCart = async (productId: string, inventoryProductId: string, selectedCuttingStyle?: string) => {
    ...
    if (!isAuthenticated || !user?.id) {
      toast.error('Please log in to manage your Enquiry List');
      // Automatically redirect to signin page after a short delay to show toast
      if (typeof window !== 'undefined') {
        setTimeout(() => {
          window.location.href = '/signin';
    ...
    try {
      ...
      toast.error('Failed to remove item from Enquiry List');
    }
  };
    ...
    try {
      ...
      if (error && typeof error === 'object' && 'response' in error) {
        ...
        if (axiosError.response?.status === 404) {
          ...
          await loadCart();
          toast.info('Enquiry List updated from server', {
            duration: 2000,
          });
          return;
        }
      ...
      setItems(previousItems);
      toast.error('Failed to update Enquiry List');
    }
  };
  ...
  const clearCart = async () => {
    ...
    try {
      ...
      toast.error('Failed to clear Enquiry List');
    }
  };
