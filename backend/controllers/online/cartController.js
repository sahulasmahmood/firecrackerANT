...
const addToCart = async (req, res) => {
  try {
    const { userId, inventoryProductId, quantity = 1 } = req.body;
...
const updateCartItem = async (req, res) => {
  try {
    ...
    if (!existingItem) {
      console.log('[updateCartItem] Cart item not found');
...
const removeFromCart = async (req, res) => {
  try {
    ...
    console.log('Remove from cart request:', { userId, inventoryProductId });
...
const syncCart = async (req, res) => {
  try {
    ...
    for (const item of items) {
      const { inventoryProductId, quantity } = item;
      ...
      if (existingItem) {
        ...
        if (!category) {
          console.error(`❌ Category not found during sync: ${product.category}`);
