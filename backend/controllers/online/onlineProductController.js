...
const createOnlineProduct = async (req, res) => {
  try {
    ...
    const preparedData = {
      ...
      isGreenCracker: Boolean(productData.isGreenCracker ?? true),
      soundLevel: productData.soundLevel || "Medium",
      pesoApproved: Boolean(productData.pesoApproved ?? true),
      bariumFree: Boolean(productData.bariumFree ?? true),
...
const updateOnlineProduct = async (req, res) => {
  try {
    ...
    if (updateData.cuttingStyles !== undefined) {
      delete updateData.cuttingStyles;
    }
